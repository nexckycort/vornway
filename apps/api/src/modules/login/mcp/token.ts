...
function resolveTokenSecret(): string {
  return process.env.MCP_TOKEN_SECRET ?? process.env.BETTER_AUTH_SECRET ?? 'dev-mcp-token-secret';
}
...
export function verifyMcpAccessToken(
  ...
  try {
    const payload = JSON.parse(base64UrlDecode(encodedPayload)) as McpTokenPayload;
