...
import { goalErrors } from './goals.errors';
import type {
...
export function createGoalOperations(): GoalOperations {
  return {
    ...
    update: async (input) => {
      ...
      if (input.name !== undefined) {
        ...
        if (!name) throw goalErrors.nameRequired();
        data.title = name;
      }
      ...
      if (input.targetAmount !== undefined) {
        if (input.targetAmount <= 0) {
          throw goalErrors.targetAmountInvalid();
        }
      ...
      if (input.installmentCount !== undefined) {
        if (input.installmentCount <= 0) {
          throw goalErrors.installmentsInvalid();
        }
      ...
      if (input.installmentAmount !== undefined) {
        if (input.installmentAmount !== null && input.installmentAmount <= 0) {
          throw goalErrors.monthlyInstallmentInvalid();
        }
    ...
    addContribution: async ({
      ...
      if (amount <= 0) {
        throw goalErrors.contributionAmountInvalid();
      }
      ...
      if (!actorMember) {
        throw goalErrors.accessDenied();
      }

      if (actorMember.role !== 'admin') {
        throw goalErrors.contributionCreateForbidden();
      }
      ...
      if (!contributionMember) {
        throw goalErrors.participantInvalid();
      }
    ...
    deleteContribution: async ({ userId, goalId, contributionId }) => {
      ...
      if (actorMember.role !== 'admin') {
        throw goalErrors.contributionDeleteForbidden();
      }
      ...
      if (!contribution) {
        throw goalErrors.contributionNotFound();
      }
    ...
    create: async (input) => {
      if (!input.name.trim()) {
        throw goalErrors.nameRequired();
      }

      if (input.targetAmount <= 0 || input.installmentCount <= 0) {
        throw goalErrors.amountAndInstallmentsInvalid();
      }
