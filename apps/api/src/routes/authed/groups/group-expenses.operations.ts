...
} from './expense-attachment.storage';
...
type ExpenseSummaryMoneyRow = {
  currency: string;
  total: number | string | null;
};

function memberExpenseDateSql(input: { startDate?: string; endDate?: string }) {
...
function memberExpenseCategorySql(input: {
  categoryId?: string;
  uncategorized?: boolean;
}) {
  if (input.categoryId) {
    return Prisma.sql`and e."categoryId" = ${input.categoryId}`;
  }

  if (input.uncategorized) {
    return Prisma.sql`and e."categoryId" is null`;
  }

  return Prisma.empty;
}

function memberExpenseScopeSql(input: {
  memberId: string;
  paidOnly?: boolean;
}) {
  if (input.paidOnly) {
    return Prisma.sql`
      and (
        e."paidById" = ${input.memberId}
        or exists (
          select 1
          from expense_payer p_scope
          where p_scope."expenseId" = e.id
            and p_scope."memberId" = ${input.memberId}
        )
      )
    `;
  }
  ...
  return Prisma.sql`
    and (
      ...
      or exists (
        select 1
        from expense_participant ep_scope
        where ep_scope."expenseId" = e.id
          and ep_scope."memberId" = ${input.memberId}
      )
    )
  `;
}

function memberExpenseSql(input: {
  groupId: string;
  memberId: string;
  paidOnly?: boolean;
  categoryId?: string;
  ...
  return Prisma.sql`
    ...
    and (
      e.notes is null
      or e.notes not like '%[DELETED]%'
    )
    ${memberExpenseScopeSql(input)}
    ${memberExpenseCategorySql(input)}
    ${memberExpenseDateSql(input)}
  `;
}
...
export function createGroupExpensesHealth() {
  return {
    ...
    listGroupMemberExpenses: async ({
      ...
      const summarySql = memberExpenseSql({
        groupId,
        memberId,
        paidOnly,
        categoryId,
      ...
      const [total, rows, spentRows, grossPaidRows] = await Promise.all([
        db.expense.count({ where }),
        db.expense.findMany({
          where,
          ...(cursor ? { cursor: { id: cursor }, skip: 1 } : {}),
        ...
        db.$queryRaw<ExpenseSummaryMoneyRow[]>`
          select e.currency, sum(ep.share)::float as total
        ...
        `,
        db.$queryRaw<ExpenseSummaryMoneyRow[]>`
          select paid.currency, sum(paid.total)::float as total
          from (
            select e.currency, p.amount as total
            ...
            select e.currency, e.amount as total
            from expense e
            where e."paidById" = ${memberId}
              and ${summarySql}
              and not exists (
                select 1
                from expense_payer p
      ...
      const nextCursor = hasNextPage
        ? (pageRows[pageRows.length - 1]?.id ?? null)
        : null;
      const spentByCurrency = spentRows.reduce<Record<string, number>>(
        (accumulator, row) => {
          const share = Number(row.total ?? 0);
      ...
      const grossPaidByCurrency = grossPaidRows.reduce<Record<string, number>>(
        (accumulator, row) => {
          const payerAmount = Number(row.total ?? 0);
    ...
    getGroupExpense: async ({
      ...
      if (!expense) {
        throw groupErrors.expenseNotFound();
      }
    ...
    createExpense: async ({
      ...
      if (normalizedExpenseId) {
        ...
        if (existingExpense) {
          if (existingExpense.groupId !== groupId) {
            throw groupErrors.expenseIdConflict();
          }
      ...
      if (validPayerIds.length === 0) {
        throw groupErrors.expensePayerInvalid();
      }
      ...
      if (categoryId && !category) {
        throw groupErrors.expenseCategoryInvalid();
      }
      ...
      if (
        ...
        throw groupErrors.expenseParticipantsInvalid();
      }
      ...
      const expense = await db.$transaction(async (tx) => {
        ...
        if (lineItems && lineItems.length > 0) {
          ...
          if (compositeItems.length !== lineItems.length) {
            throw groupErrors.expenseLineItemsInvalid();
          }
    ...
    settleDebt: async ({
      ...
      if (!fromMember || !toMember || fromMember.id === toMember.id) {
        throw groupErrors.settlementParticipantsInvalid();
      }
